...
import { CheckCircle2, FileUp, CheckSquare, Square, Lock } from 'lucide-react';
...
interface PageSelectorProps {
  ...
  isLoggedIn: boolean;
  onLogin: () => void; // Kept for prop compatibility but handled in Col 2 now
}
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-colors">
      ...
      <div className="p-5 border-b border-slate-200 dark:border-slate-800 shrink-0 bg-slate-50 dark:bg-slate-950">
        ...
        <label className={`
          flex items-center justify-center gap-2 w-full py-6 px-4 rounded-xl border-2 border-dashed 
        ...
        `}>
          <FileUp className={`w-6 h-6 ${isLoggedIn ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400'}`} />
